...
EXPLAIN SELECT COUNT(*)
        FROM READERS WHERE FIRSTNAME = 'JOHN';

CREATE INDEX TITLE ON BOOKS (TITLE);

CREATE INDEX READERS_DETAILS ON READERS(FIRSTNAME, LASTNAME);
...
DELIMITER $$
CREATE FUNCTION setBestseller(rented INT) RETURNS BOOLEAN
BEGIN
    DECLARE BOOKSREAD INT;
    DECLARE INFO BOOLEAN DEFAULT FALSE;
    DECLARE DAYS INT;
    DECLARE days_rented DECIMAL(4,2);
    IF rented >= 2 AND THEN
        SET INFO = TRUE;
END IF;
...
SET days_rented = BOOKSREAD / DAYS * 30;
SELECT BOOKS
RETURN info;
END $$
...
UPDATE BOOKS SET BESTSELLERS = setBestseller(BOOKSRENTED, CURTIME())
WHERE BOOK_ID = BK_ID;
COMMIT;
END IF;
END WHILE;
